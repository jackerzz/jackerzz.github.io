---
title: ui自动化测试
tags:
  - 测试平台构建
categories:
  - 自动化测试 
date: 2022-07-12 22:30:34
---

# ui自动化

- [metersphere--使用 UI 测试](https://metersphere.io/docs/quick_start/ui_test/) 
- [AutomationTest](https://github.com/yanchunhuo/AutomationTest)
- [DemoUI-python3 + selenium3 + PO + yaml + ddt + unittest等技术编写成基础测试框架](https://github.com/yingoja/DemoUI)
# 代码化

## 常见的低代码平台
- [nocodb](https://github.com/nocodb/nocodb)
- [n8n](https://github.com/n8n-io/n8n)
- [appsmith](https://github.com/appsmithorg/appsmith)
- [jeecg-boot](https://github.com/jeecgboot/jeecg-boot)
- [budibase](https://github.com/Budibase/budibase)
- [amis](https://github.com/baidu/amis)
- [superset](https://github.com/apache/superset)

## 前端是数据管理平台（cms）
- [strapi](https://github.com/strapi/strapi)
- [umami](https://github.com/mikecao/umami)
- [appwrite](https://github.com/appwrite/appwrite)
- [amis-react-node](https://github.com/YalongYan/amis-react-node.git) 
- [react-amis-admin](https://github.com/tcly861204/react-amis-admin.git)
- [amis-admin-vue](https://github.com/h7ml/amis-admin-vue.git)
- [amis-react-starter](https://github.com/aisuda/amis-react-starter.git)
- [amis_low_code](https://github.com/jackerzz/amis_low_code.git)

# 分布式多任务selenium

- [docker-selenium](https://github.com/SeleniumHQ/docker-selenium)

# chrome 插件开发

- [使用 selenium 向 chrome 添加扩展](http://www.smartjava.org/content/website-scraping-using-selenium-docker-and-chrome-extensions/)
- [Chrome插件(扩展)开发全攻略](https://cloud.tencent.com/developer/article/1667242)
- [一个为 HTML 元素创建高效且健壮的 CSS 选择器的库 optimal-select](https://github.com/autarc/optimal-select)

## 使用教程

- [Debug Selenium Tests in Docker using VNC Viewer](https://www.youtube.com/watch?v=SCHs3XMf1yY&t=116s)
- [realvnc-client](https://www.realvnc.com/en/connect/download/viewer/)

# vnc 模块

- [react-vnc](https://github.com/roerohan/react-vnc)
- [noVNC](https://github.com/novnc/noVNC#quick-start)
- [noVNC-docs](https://novnc.com/info.html)
- [vnv-javascript](https://www.realvnc.com/en/developer/docs/latest/api/javascript/)
- [vnv-javascript](https://www.realvnc.com/en/developer/docs/latest/api/python/index.html)


# ui自动化参考资料

- [selene-Selenium](https://github.com/yashaka/selene)
- [robotframework](https://robotframework.org/)
- [robotframework-gihub](https://github.com/robotframework/robotframework)
- [测试开发](https://github.com/china-testing/python-api-tesing#web-ui%E6%B5%8B%E8%AF%95%E8%87%AA%E5%8A%A8%E5%8C%96)
- [Web UI 自动化系统](https://blog.51cto.com/u_15333574/5307337)

# robotframework的衍生品

## rpa

- [rpa-gihub](https://robotframework.org/rpa/)
- [rpa-docs](https://robotframework.org/robotframework/latest/RobotFrameworkUserGuide.html#why-robot-framework)
- [RPA-Python](https://github.com/tebelorg/RPA-Python)

##  web,接口测试

- [SeleniumLibrary-docs](https://github.com/robotframework/SeleniumLibrary/)
- [robotframework-WebDemo](https://github.com/robotframework/WebDemo)
- [WebDemo-page](http://robotframework.org/WebDemo/)
- [robotframework-zoomba](https://github.com/Accruent/robotframework-zoomba)

## 视觉测试

- [WatchUI--视觉测试](https://github.com/Tesena-smart-testing/WatchUI)
- [WatchUI--视觉测试](https://procesor2017.github.io/WatchUI/#Compare%20Images)


## web 多机验证
- [selenium-grid](https://www.dilatoit.com/zh/2021/03/30/liulanqijianrongxingceshi-selenium-grid.html)
